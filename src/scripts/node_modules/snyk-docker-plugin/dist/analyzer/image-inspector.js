...
async function pullWithDockerBinary(docker, targetImage, saveLocation, username, password, platform) {
    ...
    try {
        ...
        debug(`couldn't pull ${targetImage} using docker binary: ${JSON.stringify(err)}`);
        if (err.stderr &&
        ...
        if (err.message &&
            err.message.includes('"--platform" is only supported on a Docker daemon with experimental features enabled')) {
            throw err;
        }
